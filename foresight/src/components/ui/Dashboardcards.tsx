...
export function DashboardCard({
  ...
  return (
    <div className="relative w-full group cursor-default">
      ...
      <div className="absolute inset-0 rounded-2xl bg-linear-to-b from-slate-100/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none dark:from-zinc-900/50" />

      {/* Main Card Element */}
      <div className="relative flex flex-col justify-between min-h-42 p-4 sm:p-5 rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03),0_4px_12px_-1px_rgba(15,23,42,0.02)] transition-all duration-300 group-hover:border-slate-300 group-hover:shadow-[0_12px_30px_-4px_rgba(15,23,42,0.06)] group-hover:-translate-y-0.5 dark:border-zinc-800 dark:bg-zinc-950 dark:group-hover:border-zinc-700 dark:group-hover:shadow-none">
        {/* Top Content Row */}
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-col gap-3 min-w-0 flex-1">
            {/* Title: Muted, professional typography */}
            <span className="text-[11px] xl:text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 truncate">
              {title}
            </span>
            {/* Value: Mapped explicitly across sm (2xl), lg (2xl), and xl (3xl) to fit perfectly at 1366px/1368px */}
            <span className="text-3xl sm:text-2xl lg:text-[24px]  font-extrabold tracking-tight text-slate-900 dark:text-zinc-50 truncate">
              {data}
            </span>
          </div>

          {/* Icon Wrapper: Crisp institutional container that expands on hover */}
          <div className="p-1.5 rounded-xl border border-slate-100 bg-slate-50 text-slate-600 shadow-[inset_0_1px_1px_rgba(255,255,255,1)] transition-all duration-300 group-hover:scale-105 group-hover:bg-slate-900 group-hover:text-white dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400 dark:group-hover:bg-zinc-100 dark:group-hover:text-zinc-900 shrink-0">
            {icon ? (
              icon
            ) : (
              <LucideCircleDollarSign className="w-4 h-4 sm:w-4 sm:h-5" />
            )}
          </div>
        </div>
        ...
        <div className="mt-auto w-full">
          {/* Divider Line */}
          <div className="h-px w-full bg-slate-100 dark:bg-zinc-900 mb-3" />
          ...
          <div className="flex items-center justify-between">
            {trend ? (
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold border truncate ${
                    trend.isPositive
