...
export function DashboardCard({ title, data, icon, trend }: DashboardCardProps) {
  return (
    <div className="relative w-full group cursor-default">
      ...
      {/* Main Card Element - Changed h-44 to min-h-[10.5rem] and reduced padding from p-6 to p-4 sm:p-5 */}
      <div className="relative flex flex-col justify-between min-h-[10.5rem] p-4 sm:p-5 rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.03),0_4px_12px_-1px_rgba(15,23,42,0.02)] transition-all duration-300 group-hover:border-slate-300 group-hover:shadow-[0_12px_30px_-4px_rgba(15,23,42,0.06)] group-hover:-translate-y-0.5 dark:border-zinc-800 dark:bg-zinc-950 dark:group-hover:border-zinc-700 dark:group-hover:shadow-none">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex flex-col gap-1 min-w-0">
            ...
            {/* Value: Added responsive font sizing (xl to 2xl to 3xl) to safeguard the container limits */}
            <span className="text-xl sm:text-2xl xl:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-zinc-50 truncate">
              {data}
            </span>
          </div>

          {/* Icon Wrapper: Crisp institutional container that expands on hover */}
          <div className="p-2.5 rounded-xl border border-slate-100 bg-slate-50 text-slate-600 shadow-[inset_0_1px_1px_rgba(255,255,255,1)] transition-all duration-300 group-hover:scale-105 group-hover:bg-slate-900 group-hover:text-white dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400 dark:group-hover:bg-zinc-100 dark:group-hover:text-zinc-900 flex-shrink-0">
            {icon ? icon : <LucideCircleDollarSign className="w-4 h-4 sm:w-5 sm:h-5" />}
          </div>
        </div>
