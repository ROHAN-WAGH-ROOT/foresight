...
// ------------------------------------------------------------------
// Optional prop: lets a parent layout react to the collapse state so
// it can adjust the main content's margin/padding (see requirement #5).
// If you don't need this, you can drop the prop and the useEffect call.
// ------------------------------------------------------------------
...
const Sidebar = ({ onCollapseChange }: SidebarProps) => {
  ...
  // ------------------------------------------------------------------
  // STATE INJECTION (Requirement #1)
  // isCollapsed controls whether the sidebar shows full labels or
  // shrinks down to an icon-only rail.
  // ------------------------------------------------------------------
  const [isCollapsed, setIsCollapsed] = useState(false);

  // Notify parent whenever the collapse state changes, so the main
  // content area can adjust its margin (Requirement #5).
  useEffect(() => {
    onCollapseChange?.(isCollapsed);
  }, [isCollapsed, onCollapseChange]);

  // ------------------------------------------------------------------
  // TOGGLE LOGIC (Requirement #2)
  // Simple flip of the boolean state, triggered by the toggle button.
  // ------------------------------------------------------------------
  ...
  return (
    // ------------------------------------------------------------------
    // LAYOUT & STYLING (Requirement #3)
    // Width toggles between a full 288px (w-72) sidebar and a slim
    // 80px (w-20) icon rail. `transition-all duration-300 ease-in-out`
    // animates the width change smoothly.
    // ------------------------------------------------------------------
    <div
      className={`relative h-screen border-r border-gray-200/80 bg-gradient-to-b from-gray-50/50 to-white py-6 select-none flex flex-col justify-between dark:border-zinc-800/80 dark:from-zinc-950 dark:to-zinc-900 transition-all duration-300 ease-in-out ${
        isCollapsed ? "w-20 px-2" : "w-62 px-4"
      }`}
    >
      {/* Decorative Blur Background Accents */}
      <div className="absolute top-0 left-0 w-32 h-32 bg-blue-400/10 rounded-full blur-3xl pointer-events-none dark:bg-blue-500/5" />
      ...
      {/* ------------------------------------------------------------------
          TOGGLE BUTTON (Requirement #2)
          Placed at the top-right edge of the sidebar so it's reachable
          in both collapsed and expanded states.
      ------------------------------------------------------------------ */}
      {/* <button
        onClick={toggleSidebar}
        className="absolute -right-3 top-8 z-10 flex items-center justify-center w-6 h-6 rounded-full border border-gray-200 bg-white shadow-md text-gray-500 hover:text-gray-900 hover:scale-110 transition-all duration-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100"
      ...
      <div className="relative flex flex-col gap-6">
        ...
        <div onClick={toggleSidebar}
          className={`flex items-center gap-2.5 h-16 group cursor-pointer ${
            isCollapsed ? "justify-center px-0" : "px-2"
          }`}
        >
          <div  className="flex items-center justify-center w-9 h-9 shrink-0 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 shadow-sm shadow-blue-500/30 transition-transform duration-500 group-hover:rotate-12 dark:from-blue-600 dark:to-purple-600">
            <Compass   className="w-5 h-5 text-white" />
          </div>

          {/* ------------------------------------------------------------------
              CONTENT HANDLING (Requirement #4)
              Labels are conditionally rendered (not just visually hidden)
              so they don't take up layout space or get read by
          ...
          {!isCollapsed && (
            <div className="flex flex-col overflow-hidden whitespace-nowrap">
              <span className="text-xl font-black tracking-tight font-sans text-gray-900 dark:text-zinc-50">
                Fore
                <span className="bg-gradient-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent dark:from-blue-400 dark:to-purple-400">
                  Sight
                </span>
              </span>
        ...
        {/* User Glassmorphic Profile Card */}
        <div
          className={`relative overflow-hidden rounded-2xl border border-gray-200/60 bg-white/60 backdrop-blur-md shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] dark:border-zinc-800/60 dark:bg-zinc-900/40 transition-all duration-300 ${
        ...
        >
          ...
          >
            {!isCollapsed && (
              <div className="flex flex-col mt-4 space-y-0.5 text-left overflow-hidden whitespace-nowrap">
                <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400 dark:text-zinc-500">
                ...
                <span className="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-purple-400 capitalize">
                    {user?.username || "Rohan !"}
                  </span>
        ...
        <div className="flex flex-col gap-1 mt-2 text-left">
          {/* {!isCollapsed && (
            <span className="px-4 text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-zinc-500 mb-2 overflow-hidden whitespace-nowrap">
              Workspace
            </span>
          )} */}
          <nav className="flex flex-col gap-1.5">
            ...
            <NavLink
              to="/risk"
              className={getNavLinkClass("/risk")}
              title={isCollapsed ? "Risk Audit" : undefined}
            >
              <ShieldAlert className="w-4 h-4 shrink-0" />
              {!isCollapsed && <span className="whitespace-nowrap">Risk Audit</span>}
              {!isCollapsed && location.pathname !== "/risk" && (
                <div className="absolute right-3 w-1 h-1 rounded-full bg-blue-500 opacity-0 group-hover:opacity-100 transition-opacity dark:bg-blue-400" />
              )}
            </NavLink>
            ...
            <NavLink
              to="/loans"
              className={getNavLinkClass("/loans")}
              title={isCollapsed ? "Loan Portfolio" : undefined}
            >
              <CreditCard className="w-4 h-4 shrink-0" />
              {!isCollapsed && <span className="whitespace-nowrap">Loan Portfolio</span>}
              {!isCollapsed && location.pathname !== "/loans" && (
                <div className="absolute right-3 w-1 h-1 rounded-full bg-blue-500 opacity-0 group-hover:opacity-100 transition-opacity dark:bg-blue-400" />
              )}
            </NavLink>
       ...
       {/* <div
          className={`relative overflow-hidden rounded-2xl border mt-40 border-gray-200/60 bg-white/60 backdrop-blur-md shadow-[0_2px_12px_-3px_rgba(0,0,0,0.04)] dark:border-zinc-800/60 dark:bg-zinc-900/40 transition-all duration-300 ${
            isCollapsed ? "p-2 flex flex-col items-center gap-2" : "p-4"
          }`}
        >
          <div
      ...
      <div className="flex flex-col gap-4 text-left">
        ...
        <button
          ...
          className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm text-rose-600 hover:bg-rose-50 hover:text-rose-700 dark:text-rose-400 dark:hover:bg-rose-950/20 dark:hover:text-rose-450 transition-all duration-300 w-full cursor-pointer ${
            isCollapsed ? "justify-center px-0 text-left" : "text-left"
          }`}
        >
        ...
        {/* Subtle Footer */}
        {!isCollapsed && (
          <div className="px-4 text-[11px] text-gray-400 dark:text-zinc-600 font-medium overflow-hidden whitespace-nowrap">
            v1.0.0 • ForeSight
          </div>
        )}
      </div>
