...
export default function MSMERiskDrillDownChart(): React.JSX.Element {
  ...
  const handleSliceClick = async (clickedSlice: any) => {
    ...
    try {
      const response: LoanDistributionItem[] =
        await getLoanRiskDistributionnTypeApi(riskCategory);
  ...
  return (
    <div className="bg-white dark:bg-slate-900/60 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs flex flex-col justify-between min-h-105 relative">
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          ...
          {selectedCategory
              ...
              : "MSME Account Segment Distribution"}
          </div>
      ...
      <div className="h-70 w-full pt-4 flex items-center justify-center relative">
        {isLoading ? (
          <div className="flex flex-col items-center gap-2 text-slate-500 dark:text-slate-400">
            <Loader2 className="w-8 h-8 animate-spin text-blue-500" />
            <span className="text-xs font-medium">
              Communicating with core servers...
            </span>
          </div>
        ) : error ? (
